...
def expected_goals(home_str: float, away_str: float) -> tuple[float, float]:
    ...
    home_lambda = base + diff * 0.8 + random.uniform(-0.3, 0.3)
    away_lambda = base - diff * 0.8 + random.uniform(-0.3, 0.3)
...
def generate_events(
    ...
    if red_minute is not None:
        ...
        if red_team == "home":
            h_lam2 = h_lam * (1 - 0.7 * scale)   # home теряет до 70% угрозы
            a_lam2 = a_lam * (1 + 0.4 * scale)    # away получает до 40% бонуса
        else:
            a_lam2 = a_lam * (1 - 0.7 * scale)
